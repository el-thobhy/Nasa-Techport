...
class TechportRemoteMediator(
...
) : RemoteMediator<Int, TechportEntity>() {
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, TechportEntity>
    ): MediatorResult {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, TechportEntity>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.projectid)
        }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, TechportEntity>): RemoteKeys? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.projectid)
        }
    }

    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, TechportEntity>): RemoteKeys? {
